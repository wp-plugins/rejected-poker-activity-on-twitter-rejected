=== Poker Activity on Twitter ===
Contributors: webmsol
Donate link: http://onlinemarketing.eu
Tags: poker, twitter, gambling
Requires at least: 2.8
Tested up to: 2.8
Stable tag: trunk

Displays the latest Twitter activity with the term "poker" in it. Great for poker-enthusiasts.

== Description ==

Displays the latest Twitter activity with the term "poker" in it. Great for poker-enthusiasts.

== Installation ==

1. Upload [poker_activity_on_twitter.php] to your [/wp-content/plugins/] directory.
2. Activate the plugin through the 'Plugins' menu in WordPress.
3. Click on 'Widgets' under 'Appearance'.
4. Drag the widget to your sidebar.

== Frequently Asked Questions ==

None yet. Be the first to ask! :)

== Screenshots ==

None yet.

== Changelog ==

= 1.0.2 =
* Plugin lets author know when you've activated or deactivated the plugin. No sensitive information is sent. See source code to verify this.

= 1.0.1 =
* Fixed default stylesheet option.

= 1.0 =
* Initial version.